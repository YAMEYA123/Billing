...
- 当前实现已经是“批量消费、按 key 汇聚、批量入库”，后续方案不把逐条写库改造当作收益来源。
...
这里的性能优化重点是：先确认批次汇聚结果是否真正以少量批量 SQL/事务入库，再稳定批量 Upsert 和事务边界、处理多 Pod/Rebalance 的幂等、避免窗口固化竞态，以及提升话单出口吞吐；“减少批次之间对同一聚合行的重复更新”只有在另行采用窗口级持续汇聚时才成立，不是当前默认改造。
...
- [11-按domain分批割接简版.md](./11-按domain分批割接简版.md)
